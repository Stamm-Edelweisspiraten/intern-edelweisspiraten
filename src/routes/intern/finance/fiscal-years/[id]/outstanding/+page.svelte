...
<svelte:head><title>Offene Posten {data.year.year} - Kasse</title></svelte:head>

<div class="space-y-8">
    <PageHeader
        title={`Offene Posten ${data.year.year}`}
        eyebrow="Kasse"
        subtitle="Noch nicht ausgeglichene Forderungen dieses Geschäftsjahres."
        back={{ href: `/intern/finance/fiscal-years/${data.year.id}` }}
    >
        {#snippet actions()}
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <StatTile label="Überfällige Posten" value={data.overdueCount} tone="danger" icon="exclamation-triangle" />
